  ...
  try {
    ...
    if (searchQuery) {
      ...
      getSeriesRes = getSeriesRes.filter((series) => {
        ...
        const genres = Array.isArray(series.genre)
          ? series.genre.join(" ").toLowerCase()
          : "";
    ...
    document
      ...
      .forEach(async (bookmarkEl, index) => {
        const series = getSeriesRes[index];
    ...
    document.querySelectorAll(".genre-option").forEach((genre) => {
      genre.addEventListener("click", function () {
        ...
        if (this.classList.contains("selected")) {
          ...
          genreSelection = genreSelection.filter(
    ...
    function filterSeriesByGenres() {
      ...
      seriesBoxes.forEach((box, index) => {
        ...
        const seriesGenres = Array.isArray(series.genre)
          ? series.genre
      ...
      seriesBoxes.forEach((box, index) => {
        ...
        try {
          releaseYear = series.releaseDate
            ? new Date(series.releaseDate).getFullYear().toString()
            : "";
        } catch (e) {
          releaseYear = "";
        }
    ...
    // Add chapter count filter functionality
    let chapterCountFilter = ""; 
    document.querySelectorAll("#chapter-count-filter-dropdown")
      .forEach((dropdown) => {
        dropdown.querySelectorAll(".dropdown-option").forEach((option) => {
          option.addEventListener("click", function () {
            const optionText = this.textContent.trim();

            // Remove 'selected' class from all chapter count options
            dropdown.querySelectorAll(".dropdown-option").forEach((opt) => {
              opt.classList.remove("selected");
            });

            // If clicking the same option that's already selected, clear the filter
            if (chapterCountFilter === optionText) {
              chapterCountFilter = "";
            } else {
              // Add selected class to clicked option and update chapterCountFilter
              this.classList.add("selected");
              chapterCountFilter = optionText;
            }

            console.log("Selected chapter count filter:", chapterCountFilter);
            filterSeriesByChapterCount();
          });
        });
      });

    function filterSeriesByChapterCount() {
      const seriesBoxes = document.querySelectorAll(".series-box");
      let visibleCount = 0;

      if (chapterCountFilter === "" && !searchQuery) {
        // Show all series if no chapter count filter is selected and no search query
        seriesBoxes.forEach((box) => {
          box.style.display = "block";
        });
        nothingFound.style.display = "none";
      ...
      seriesBoxes.forEach((box, index) => {
        ...
        // Get chapter count (assuming it's stored in series.chapterCount or similar)
        const chapterCount = parseInt(series.chapters.length) || 0;

        // Check if series matches the chapter count filter
        let matchesChapterCount = true;
        if (chapterCountFilter !== "") {
          switch (chapterCountFilter) {
            case "< 10":
              matchesChapterCount = chapterCount < 10;
              break;
            case "10-20":
              matchesChapterCount = chapterCount >= 10 && chapterCount <= 20;
              break;
            case "20-30":
              matchesChapterCount = chapterCount >= 20 && chapterCount <= 30;
              break;
            case "30-40":
              matchesChapterCount = chapterCount >= 30 && chapterCount <= 40;
              break;
            case "40-50":
              matchesChapterCount = chapterCount >= 40 && chapterCount <= 50;
              break;
            case "50-60":
              matchesChapterCount = chapterCount >= 50 && chapterCount <= 60;
              break;
            case "60-70":
              matchesChapterCount = chapterCount >= 60 && chapterCount <= 70;
              break;
            case "70-80":
              matchesChapterCount = chapterCount >= 70 && chapterCount <= 80;
              break;
            case "80-90":
              matchesChapterCount = chapterCount >= 80 && chapterCount <= 90;
              break;
            case "90-100":
              matchesChapterCount = chapterCount >= 90 && chapterCount <= 100;
              break;
            case "100+":
              matchesChapterCount = chapterCount > 100;
              break;
            default:
              matchesChapterCount = true;
          }
        }

        // If we have a search query, only show if it matches the search AND chapter count filters
        const matchesSearch =
          !searchQuery || seriesMatchesSearch(series, searchQuery);

        const shouldDisplay = matchesChapterCount && matchesSearch;
